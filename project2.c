...
extern struct _led_state led_state[7];
...
static void example_set_all_leds(uint8_t onoff)
{
    for (int i = 0; i < 7; i++) {
        board_led_operation(led_state[i].pin, onoff);
    }
}
                                     ...
                                     esp_ble_mesh_msg_ctx_t *ctx, uint8_t onoff)
{
    ...
    } else if (ESP_BLE_MESH_ADDR_IS_GROUP(ctx->recv_dst) || ctx->recv_dst == 0xFFFF) {
        for (i = 0; i < elem_count; i++) {
            led = &led_state[i];
            board_led_operation(led->pin, onoff);
        }
